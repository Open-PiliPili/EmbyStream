...
<template>
  <AppWorkspaceShell
    ...
    :eyebrow="t('common.disclaimer')"
    :title="t('disclaimer.title')"
  >
    <GlassPanel class="disclaimer-card" tone="warm">
      <div class="disclaimer-card__links">
        <a :href="claudeUrl" rel="noopener noreferrer" target="_blank">
          <svg
            aria-hidden="true"
            class="disclaimer-card__brand-icon"
            viewBox="0 0 24 24"
          >
            <path
              clip-rule="evenodd"
...
.disclaimer-card__links {
  display: flex;
  flex-wrap: wrap;
  gap: 0.75rem;
}

.disclaimer-card__links a {
  display: inline-flex;
  align-items: center;
  gap: 0.55rem;
  min-height: 2.45rem;
